...
if __name__ in ["__main__"]:
    ...
    file_name = "centroid_rotation_%s_17keV_%d.dat" % (rotation_axis, crystal_number)
    if do_calculate:
        if rotation_axis == 'x':
            OFFSET = numpy.linspace(-50e-6, 50e-6, 11)
        elif rotation_axis == 'y':
            OFFSET = numpy.linspace(-10000e-6, 10000e-6, 11)
    ...
    plot(1e6 * OFFSET, 1e6 * CEN_x,
         ...
         xtitle="rotation [urad]", ytitle="centroid [um]", legend=["x", "z"],  yrange=[-1500, 1500], grid=1, show=0)

    plot(numpy.degrees(OFFSET), 1e6 * CEN_x,
         numpy.degrees(OFFSET), 1e6 * CEN_z,
         xtitle="rotation [deg]", ytitle="centroid [um]", legend=["x", "z"],  yrange=[-1500, 1500], grid=1, show=1)
