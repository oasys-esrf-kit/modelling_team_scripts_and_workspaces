...
\begin{equation}
  ...
  \approx 1268\; E_e^2\,[\text{GeV}^2]\; B^2\,[\text{T}^2]\; L\,[\text{m}]\; I\,[\text{A}]
  \quad[\text{W}].
  \label{eq:PBM}
\end{equation}
...
\begin{equation}
  ...
  = \frac{1.602\times10^{-19}\times 1.463\times10^{13}}
         {6\pi\times 8.854\times10^{-12}\times 3.3356}
  = \frac{2.344\times10^{-6}}{5.567\times10^{-10}}
  = 4211 \quad [\text{W\,rad}^{-1}\text{GeV}^{-3}\text{T}^{-1}\text{A}^{-1}],
\end{equation}
giving
\begin{equation}
  \boxed{\frac{dP}{d\theta} = 4211\cdot E_e^3[\text{GeV}^3]\cdot B[\text{T}]
         \cdot I[\text{A}] \quad [\text{W/rad}]}
  = 4.211\cdot E_e^3\cdot B\cdot I \quad [\text{W/mrad}].
  \label{eq:dPdtheta_larmor}
\end{equation}
This agrees with the spectral-formula result of $4.221$\,W/mrad
(Eq.\,\ref{eq:dPdtheta}) to within \textbf{0.4\%}, the residual being purely
rounding of the fundamental constants.  The two routes --- relativistic Larmor
...
\begin{align}
  ...
  \nonumber\\
    &= \frac{2N \times 4.229\times10^3}{3.3356}\; E_e^2\; I
       \int_0^{\lambda_u/2} B_0^2\cos^2\!\left(\frac{2\pi s}{\lambda_u}\right)ds
  \nonumber\\
    &= \frac{2N \times 4.229\times10^3}{3.3356}\; E_e^2\; I \cdot
       \frac{B_0^2\,\lambda_u}{4}.
  \label{eq:approach1}
\end{align}
This gives coefficient $634$\,W\,T$^{-2}$\,m$^{-1}$\,GeV$^{-2}$\,A$^{-1}$, in
excellent agreement with the LBL reference value \cite{kim1989}:
\begin{equation}
  \boxed{P\,[\text{W}] = 633\; B_0^2\,[\text{T}^2]\; L\,[\text{m}]\;
         E_e^2\,[\text{GeV}^2]\; I\,[\text{A}]},
  \label{eq:P633}
...
overestimates by exactly $\pi/4$, because $dP/d\theta \propto B$ 
...
at full aperture (exact total power). The check above confirms both limits, and for the CPMU18 slit (u=0.27) it differs from the rectangle by only 1.2\% — confirming why the small-slit results were already excellent.
...
\begin{itemize}
  ...
  \item A \textbf{hybrid implementation} selects automatically between the two
        approaches based on whether the slit is narrower or wider than $2K/\gamma$. The correction for smoothing the transition has not been implemented.
