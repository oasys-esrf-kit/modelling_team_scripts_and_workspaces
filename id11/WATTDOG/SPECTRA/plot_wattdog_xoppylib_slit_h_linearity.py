...
RESULTS_FILE = "plot_wattdog_xoppylib_slit_h_linearity.json"
...
slit_v_mm = 1.0
slit_h_vals = np.linspace(0.001, 1.801, 5)

file_CrossSec = "CrossSec_NIST_MassEnergyAbsorption.dat"

# results[device][method][attenuator][slit_h] = (power_ll, power_bb)
...
if os.path.exists(RESULTS_FILE):
    ...
    for key, val in flat.items():
        ...
        results[device][method][att][round(slit_h, 4)] = tuple(val)
else:
    results = {}
            ...
            for slit_h in slit_h_vals:
                for att in attenuators:
                    ...
                    POWER = calculate_power(method=method, u=device, K=Kmax[device],
                                            slit_h_mm=slit_h, slit_v_mm=slit_v_mm,
                    ...
                    # results[device][method][att][round(slit_h, 4)] = (POWER[3], POWER[2])
                    results[device][method][att][round(slit_h, 4)] = (POWER[3], POWER[1]) # BRAGG absorbes everything!!
    ...
    for device in results:
        for method in results[device]:
            for att in results[device][method]:
                for slit_h, val in results[device][method][att].items():
                    flat[f"{device}|{method}|{att}|{slit_h}"] = list(val)
...
for j, device in enumerate(devices):
    for i, att in enumerate(attenuators):
        ...
        for method in methods:
            ...
            ll_vals = [results[device][method][att][round(sh, 4)][0] for sh in slit_h_vals]
            bb_vals = [results[device][method][att][round(sh, 4)][1] for sh in slit_h_vals]
            r2_ll = r2(slit_h_vals, ll_vals)
            r2_bb = r2(slit_h_vals, bb_vals)
            r2_lines.append(f'll {lbl} R²={r2_ll:.4f}')
            r2_lines.append(f'bb {lbl} R²={r2_bb:.4f}')
            a[j, i].plot(slit_h_vals, ll_vals, 'o-',  label=f'll {lbl}')
            a[j, i].plot(slit_h_vals, bb_vals, 's--', label=f'bb {lbl}')
...
for i in range(len(attenuators)):
    a[-1, i].set(xlabel='slit H / mm')

plt.suptitle('WattDog — slit H linearity at Kmax, power absorbed in LL and Bragg crystals')

plt.tight_layout()
outpng = "plot_wattdog_xoppylib_slit_h_linearity.png"
